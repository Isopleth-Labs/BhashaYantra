...
export function StenographyWorkspace({ defaultLanguage }: { readonly defaultLanguage: TypingLanguageCode }) {
  ...
  const [audioMessage, setAudioMessage] = useState("Run Voice test or import a human recording before starting");
  ...
  return (
    <section className="steno-studio steno-exam-studio">
      ...
      <div className="steno-insights">
        <article className="steno-score-card"><div className="panel-kicker"><Target /><span>TRANSCRIPTION RESULT CARD</span></div>{score ? <div className="score-summary"><div className="score-ring" style={{ "--score": `${score.accuracy * 3.6}deg` } as CSSProperties}><strong>{score.accuracy}%</strong><span>accuracy</span></div><div className="score-metrics"><span><small>Dictation speed</small><strong>{profile.dictationWpm} WPM</strong></span><span><small>Transcript speed</small><strong>{calculateTranscriptWpm(score.typedWords, transcriptionElapsed)} WPM</strong></span><span><small>Reference / typed</small><strong>{score.expectedWords} / {score.typedWords}</strong></span><span><small>Correct words</small><strong>{score.correct}</strong></span><span><small>Omitted / extra</small><strong>{score.missing} / {score.extra}</strong></span><span><small>Changed words</small><strong>{score.substitutions}</strong></span></div></div> : <div className="empty-score"><Radio /><strong>No result yet</strong><span>Complete both phases to receive word-level speed and error analysis.</span></div>}</article>
